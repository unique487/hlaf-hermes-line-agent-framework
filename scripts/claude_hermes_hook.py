...
Registered via `config/claude_hermes_settings.json` (passed to `claude` with
...
NOTE (unverified assumption): the exact stdin payload shape
(`{"session_id", "tool_name", "tool_input", ...}`) and the stdout decision
schema (`{"decision": "approve"|"block", "reason": "..."}`) follow the
documented Claude Code hooks convention as of this writing, but have not been
exercised against a real `claude` binary in this environment. Cross-check
against the installed CLI's hooks documentation on the machine that actually
runs Hermes (`claude --help`, hooks docs) before relying on this in
production — field names in particular may differ by version.

This script must be runnable standalone as a subprocess (no guarantee it
inherits the Hermes FastAPI process's Python environment beyond whatever
`claude` passes through), so it adds the repo root to `sys.path` itself and
...
# Should stay comfortably above HERMES_CONFIRM_TIMEOUT_SECONDS (default 600s)
# so the hook doesn't give up before Hermes itself times out the confirm.
_HTTP_TIMEOUT_SECONDS = 630
_DEFAULT_BASE_URL = "http://127.0.0.1:8000"


def _base_url() -> str:
    return os.environ.get("HERMES_INTERNAL_BASE_URL", _DEFAULT_BASE_URL)
...
def main() -> None:
    ...
    try:
        ...
        _block(f"無法連上 Hermes 內部確認服務,已阻擋此動作:{exc}")
        return
